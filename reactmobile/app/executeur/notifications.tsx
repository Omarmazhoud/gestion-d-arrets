...
const STATUS_MAP: Record<string, {
  ...
  OUVERTE: {
    titre: "Nouveau Ticket Disponible",
    message: (t) => `Un ticket "${t.typePanne.replace(/_/g, " ")}" est disponible. Demandé par ${t.demandeur?.nom || "un demandeur"}.`,
  ...
  REOUVERTE: {
    titre: "Ticket Réouvert",
    message: (t) => `Le ticket "${t.typePanne.replace(/_/g, " ")}" a été réouvert : "${t.commentaireVerification || "correction refusée"}".`,
  ...
  REFUSE_QUALITE: {
    titre: "Refus Qualité",
    message: (t) => `La qualité a refusé votre intervention sur le ticket "${t.typePanne.replace(/_/g, " ")}". Veuillez reprendre.`,
  ...
  EN_COURS: {
    ...
    message: (t) => `Vous intervenez actuellement sur le ticket "${t.typePanne.replace(/_/g, " ")}".`,
  ...
  A_VERIFIER_QUALITE: {
    titre: "En Attente Qualité",
    message: (t) => `Votre intervention sur "${t.typePanne.replace(/_/g, " ")}" est en cours de vérification par le service Qualité.`,
    icon: "shield-checkmark-outline",
    iconColor: "#8B5CF6",
    bgColor: "#F5F3FF",
    isNew: false,
  },
  A_VERIFIER_DEMANDEUR: {
    titre: "En Attente du Demandeur",
    message: (t) => `Le demandeur ${t.demandeur?.nom || ""} doit valider votre intervention sur "${t.typePanne.replace(/_/g, " ")}".`,
  ...
  FERMEE: {
    ...
    message: (t) => `Le demandeur ${t.demandeur?.nom || ""} a validé votre intervention. Ticket fermé avec succès !`,
...
export default function Notifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    if (!userId) return;
    try {
      const res = await axios.get(`${BASE_URL}/tickets/executeurTickets/${userId}`);
...
const styles = StyleSheet.create({
  ...
  backButton: { backgroundColor: "#F0F5FA", padding: 10, borderRadius: 14 },
  ...
  cardTitle: { fontSize: 15, fontWeight: "800", color: "#1E293B", flex: 1, marginRight: 8 },
  dot: { width: 10, height: 10, borderRadius: 5, backgroundColor: "#005A9C" },
